...
std::string makeNameOrig(const std::string& name, const bls::Id& id)
{
	const std::string suf = "_orig.txt";
	if (id.isZero()) return name + suf;
	std::ostringstream os;
	os << name << '.' << id << suf;
	return os.str();
...
{
	const std::string name = makeNameOrig(file, id);
...
int init()
{
	...
	sec.init();
	save(secFile, sec);
	const std::string name = makeName(secFile, 0);
	std::ofstream ofs(name.c_str(), std::ios::binary);
	if (!(ofs << sec.serializeToHexStr())) {
		throw cybozu::Exception("can't save") << name;
	}
	bls::PublicKey pub;
	sec.getPublicKey(pub);
	save(pubFile, pub);
	const std::string name2 = makeName(pubFile, 0);
	std::ofstream ofs2(name2.c_str(), std::ios::binary);
	if (!(ofs2 << pub.serializeToHexStr())) {
		throw cybozu::Exception("can't save") << name2;
	}
	return 0;
}
...
int sign(const std::string& m, int id)
{
	...
	bls::SecretKey sec;
	load(sec, secFile, id);
	// const std::string name = makeName(secFile, id);
	// std::ifstream ifs(name.c_str(), std::ios::binary);
	// std::string str;
	// ifs >> str;
	// sec.deserializeHexStr(str);
	bls::Signature s;
	sec.sign(s, m);
	save(signFile, s, id);
	// const std::string name2 = makeName(signFile, id);
	// std::ofstream ofs(name2.c_str(), std::ios::binary);
	// if (!(ofs << s.serializeToHexStr())) {
	// 	throw cybozu::Exception("can't save") << name2;
	// }
	return 0;
}
...
int verify(const std::string& m, int id)
{
	...
	bls::PublicKey pub;
	load(pub, pubFile, id);
	// const std::string name = makeName(pubFile, id);
	// std::ifstream ifs(name.c_str(), std::ios::binary);
	// std::string str;
	// ifs >> str;
	// pub.deserializeHexStr(str);
	bls::Signature s;
	load(s, signFile, id);
	// const std::string name2 = makeName(signFile, id);
	// std::ifstream ifs2(name2.c_str(), std::ios::binary);
	// std::string str2;
	// ifs2 >> str2;
	// s.deserializeHexStr(str2);
	if (s.verify(pub, m)) {
		puts("verify ok");
		return 0;
	} else {
...
int share(size_t n, size_t k)
{
	...
	bls::SecretKey sec;
	load(sec, secFile);
	bls::SecretKeyVec msk;			//msk contains the k coefs. of the polynomial
	sec.getMasterSecretKey(msk, k); //makes msk[0]==sec and the other entries random
	bls::SecretKeyVec secVec(n);
	bls::IdVec ids(n);
	for (size_t i = 0; i < n; i++) {
		int id = i + 1;
		ids[i] = id;
		secVec[i].set(msk, id); //evaluates polynomial with coefs. msk on point id, id>0
	}
	for (size_t i = 0; i < n; i++) {
